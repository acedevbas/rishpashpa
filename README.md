...
2. Выполните `npm start`. Скрипт соберёт фронтенд, сгенерирует `public/env.js`, запустит сервер и бота.
3. По умолчанию сервер стартует по адресу `http://localhost:3000`. Если задать `USE_HTTPS=true`, он запустится на `https://localhost:3000`
   (используется самоподписанный сертификат из `cert/`).
4. В Telegram отправьте своему боту команду `/start`. При активном HTTPS бот предложит кнопку **Открыть Ришпашпу**, иначе сообщит,
   что мини‑приложение доступно только по HTTPS.
5. Для использования собственных сертификатов укажите переменные окружения `SSL_KEY_PATH` и `SSL_CERT_PATH`.
